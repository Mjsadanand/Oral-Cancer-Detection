...
# Class labels loaded from training metadata when available.
CLASS_LABELS = {
    0: "Class_0",
    1: "Class_1"
}
...
def load_class_labels():
    """Load index->label mapping generated by the training script."""
    global CLASS_LABELS
    class_indices_file = Path(CLASS_INDICES_PATH)

    if not class_indices_file.exists():
        print(f"⚠️ Class mapping file not found at {CLASS_INDICES_PATH}. Using fallback labels.")
        return
...
def main():
    ...
    # Load class mapping saved by training to keep labels aligned.
    load_class_labels()
